  ...
  for (const offset of verticalOffsets) {
    for (const range of laneRanges) {
      const rails = {
        ...
        lanePitch: round(range.lanePitch || config.pitch),
        laneMode: range.mode,
        offset
      };
...
function candidateLaneRanges(config, index, separation, fov) {
  ...
  const ranges = [];
  const activeCount = Math.max(1, state.circuitCount);

  // Preferred multi-circuit strategy: split one global pitch grid into
  // contiguous circuit blocks. With pitch=5 and two circuits this produces
  // C1 lanes at 25,30,35... and C2 lanes continuing at 55,60,65..., so the
  // final physical spacing between neighboring vertical wires remains 5 mm
  // without making one circuit's vertical legs cross another circuit's returns.
  if (activeCount > 1 && config.pitch > 0) {
    const globalLaneCount = Math.floor((fullMax - fullMin) / config.pitch + EPS) + 1;
    const baseCount = Math.floor(globalLaneCount / activeCount);
    const remainder = globalLaneCount % activeCount;
    const blockStartIndex = index * baseCount + Math.min(index, remainder);
    const blockCount = baseCount + (index < remainder ? 1 : 0);
    if (blockCount >= 2) {
      ranges.push({
        laneMin: fullMin + blockStartIndex * config.pitch,
        laneMax: fullMin + (blockStartIndex + blockCount - 1) * config.pitch,
        lanePitch: config.pitch,
        mode: "contiguous-grid"
      });
    }

    ranges.push({
      laneMin: fullMin + index * config.pitch,
      laneMax: fullMax,
      lanePitch: config.pitch * activeCount,
      mode: "interleaved-fallback"
    });
  }

  // Fallbacks preserve the original single-circuit behavior and try nested
  // offsets if the shared global grid is impossible for unusually narrow geometry.
  ranges.push({ laneMin: fullMin, laneMax: fullMax, lanePitch: config.pitch, mode: "base" });
  const fallbackSigns = index === 0 ? [] : [-1, 1];
  for (const sign of fallbackSigns) {
    const offset = index * separation;
    ranges.push({ laneMin: fullMin + sign * offset, laneMax: fullMax + sign * offset, lanePitch: config.pitch, mode: "nested" });
  }
  return ranges;
}
...
function buildCandidate(config, index, rails, usable, fov, obstacles) {
  ...
  const lanes = generateLanes(rails.laneMin, rails.laneMax, rails.lanePitch || config.pitch, usable);
...
function routeTerminalConnection({ from, to, rails, fov, usable, obstacles, bodySegments, config, index, terminalKey, sharedPads }) {
  ...
  const validRoutes = [];
  const bodyTouchPoints = bodySegments.flatMap(segment => [segment.a, segment.b]);
  const allowedTouch = [from, to, ...bodyTouchPoints];
...
function autoFitLayout() {
  ...
  let maxRight = Math.max(...active.map(c => c.rightClearance));
  ...
  const largestPitch = Math.max(...active.map(c => c.pitch));
  const requiredGlobalLaneCount = Math.max(2, state.circuitCount * 2);
  const requiredLaneSpan = round((requiredGlobalLaneCount - 1) * largestPitch);
  const currentLaneSpan = round(state.fovWidth + maxLeft + maxRight);
  if (currentLaneSpan < requiredLaneSpan) {
    const extraRightClearance = round(requiredLaneSpan - currentLaneSpan);
    for (const c of active) c.rightClearance = round(c.rightClearance + extraRightClearance);
    maxRight = Math.max(...active.map(c => c.rightClearance));
    changes.push(`Right lane clearance increased by ${extraRightClearance} mm so ${state.circuitCount} circuit(s) have at least two lanes on one shared ${round(largestPitch)} mm grid.`);
  }
...
document.getElementById("topRegenerateLayout").addEventListener("click", () => { state.autoFitSummary = ""; regenerate(); });
document.getElementById("autoFitLayout").addEventListener("click", autoFitLayout);
document.getElementById("topAutoFitLayout").addEventListener("click", autoFitLayout);
